...
fn test_read_fuzz_corpus() {
    ...
    for file in fs::read_dir(corpus_dir()).unwrap() {
        ...
        assert_eq!(bytes[..used].to_vec(), enc);
    }
}
...
fn debug_payload() {
    assert_eq!(
        "01020304",
        format!("{:?}", Payload(vec![1, 2, 3, 4].into()))
    );
    assert_eq!(
        "01020304",
        format!("{:?}", PayloadU8(vec![1, 2, 3, 4].into()))
    );
    assert_eq!(
        "01020304",
        format!("{:?}", PayloadU16(vec![1, 2, 3, 4].into()))
    );
    assert_eq!(
        "01020304",
        format!("{:?}", PayloadU24(vec![1, 2, 3, 4].into()))
    );
}
